...
class LmaframeMultiLabelDataset(BaseDataset):
    def __init__(self,
                 ann_file,
                 pipeline,
                 data_prefix=None,
        ...
        if self.ann_file.endswith('.json'):
            ...
            # creating a csv reader object
                ...
                for row in csvreader:
                    ...
                    # get the multi lma label
                    lma_cls = np.zeros(11)
                    for idx in range(3, 3+11):
                        try:
                            lma_cls[idx-3] = int(int(row[idx])>0)
                        except:
                            lma_cls[idx-3] = 0
                    multi_label = np.argwhere(lma_cls > 0.5)
                    label = []
                    for _label in multi_label:
                        label.append(_label[0])
                    video_info['label'] = label
                    video_info['meta_label'] = []
