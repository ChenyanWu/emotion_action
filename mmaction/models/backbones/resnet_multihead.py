...
class ResNetMultiHead(nn.Module):
    ...
    def __init__(self,
        ...
        for i, num_blocks in enumerate(self.stage_blocks):
            ...
            if self.extra_head > 0 and i == len(self.stage_blocks) -1:
                for extra_id in range(self.extra_head):
                    extra_layer = make_res_layer(
                        self.block,
                        self.inplanes,
                        planes,
                        num_blocks,
                        ...
                        act_cfg=act_cfg,
                        with_cp=with_cp)
                    extra_name = f'extra{extra_id + 1}'
                    self.add_module(extra_name, extra_layer)
                    self.extra_layers.append(extra_name)
    ...
    def _load_torchvision_checkpoint(self, logger=None):
        ...
        for name, module in self.named_modules():
            if isinstance(module, ConvModule):
                ...
                if 'extra' in name:
                    for ex_id in range(self.extra_head):
                        meta_name = name.replace('extra{}'.format(ex_id+1), 'layer4')
                        meta_name = name.replace('extra{}'.format(ex_id+1), 'layer4')
                else:
                    meta_name = name
                if 'downsample' in meta_name:
                    # layer{X}.{Y}.downsample.conv->layer{X}.{Y}.downsample.0
                    original_conv_name = meta_name + '.0'
                    # layer{X}.{Y}.downsample.bn->layer{X}.{Y}.downsample.1
                    original_bn_name = meta_name + '.1'
                else:
                    # layer{X}.{Y}.conv{n}.conv->layer{X}.{Y}.conv{n}
                    original_conv_name = meta_name
                    # layer{X}.{Y}.conv{n}.bn->layer{X}.{Y}.bn{n}
                    original_bn_name = meta_name.replace('conv', 'bn')
    ...
    def forward(self, x):
        ...
        for i, layer_name in enumerate(self.res_layers):
            if i == len(self.res_layers) - 1:
                for extra_id, extra_name in enumerate(self.extra_layers):
                    extra_layer = getattr(self, extra_name)
                    outs.append(extra_layer(x))
